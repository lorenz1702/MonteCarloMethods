...
# Temperature range change temperatures
beta_c = np.log(1+ np.sqrt(max_state)) # 0.44
temperatures = np.linspace(0, 2.0, 15) #
...
def initial_state(L_param):
    return np.random.choice([min_state, max_state], size=(L_param, L_param))
...
def calculate_total_magnetization(spins_param):
    ...
    # N = Latttice
    # N = most common state
    unique, counts = np.unique(spins_param, return_counts=True)
    N_max = counts[np.argmax(counts)]
    #magnetization = max_state/(max_state-1)*(N_max/(L*L)-1/max_state)
    return max_state/(max_state-1)*(N_max/(L*L)-1/max_state)
...
for T_metro in temperatures:
    ...
    for step in range(n_th_steps_metro):
        ...
        if delta_E <= 0 or np.random.rand() < np.exp(-delta_E / T_metro):
            spins[i, j] = np.random.randint(min_state, max_state, size=1) #Changes the state
    ...
    for step in range(n_mc_steps_metro):
        ...
        if (step + 1) % measurement_interval_metro == 0:
            M_current = calculate_total_magnetization(spins) # Change magnetization calculation
...
Tc_theor = 1 / (np.log(1 + np.sqrt(max_state)))
